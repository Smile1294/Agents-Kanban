...
export interface ModelChoice { id: string; label: string; context: string }

/**
 * What the model picker should offer for this profile.
 *
 ...
 * the Claude ids name nothing at all. So a profile may declare its own ids, and
 * this turns them into picker entries.
 *
 * Labels and windows are DERIVED rather than asked for. `normaliseModel` already
 * knows that `us.anthropic.claude-haiku-4-5-20251001-v1:0` is Haiku 4.5, so a
 * Bedrock profile listing its full inference-profile ids gets the same readable
 * name and the same 200K window as first-party, from the same table the context
 * meter measures against. That is the existing rule — "a label that disagrees
 * with the meter beside it is not expressible" — extended to ids we did not
 * write. Only a genuinely unknown model falls back to its raw id, and to the
 * profile's declared window, which is the only number available for it.
 *
 * `fallback` is the built-in list, passed in rather than imported so this stays
 * a pure function of its arguments.
 */
export function modelsForProfile(
  profile: ProviderProfile,
  fallback: readonly ModelChoice[],
  normalise: (id: string) => string,
  windows: Record<string, number>,
  windowLabel: (tokens: number | undefined) => string,
): ModelChoice[] {
  if (!profile.models?.length) return [...fallback]
  return profile.models.map((id) => {
    const base = normalise(id)
    const known = fallback.find((f) => f.id === base)
    const tokens = windows[base] ?? profile.contextWindow
    return {
      id,
