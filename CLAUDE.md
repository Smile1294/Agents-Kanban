...
| Should the remote board become a data server the browser renders from? | [docs/REMOTE-REWORK.md](docs/REMOTE-REWORK.md) — research; nothing built. The blocker is that `mode`/`selectedKey` are ONE pair of host globals shared by every surface, so a remote page is a mirror and opening a chat is a round trip. Carries the effort estimate and the security analysis (localhost is not a trust boundary — CVE-2026-25253) |
