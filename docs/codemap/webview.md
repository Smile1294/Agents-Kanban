...
- 2026-09-25 · claude/self-checkout-harness-overview-cvpkyy · board OVERVIEW layout (toolbar "Columns | Overview", remembered as `disclosed['board:overview']`): `sectionOf(c)` sorts cards by what they need — needs you, running now, ready to test, queued, not started, done — `renderOverview` / `renderOverviewRow` (running rows reuse `agentRow` and register in `syncStrips`, so the fast path keeps them live; test rows show the plan summary, `verified` errors and the auto-check); columns FOLD to a 44px bar with a dot per card (`disclosed['col:<id>'] === false`), still a drop target.
...
- 2026-09-25 · claude/self-checkout-harness-overview-cvpkyy · images: `encodeForModel` re-encodes anything the host would refuse (BMP/SVG/AVIF/`image/jpg`, over 3.5MB, or over 1568px) — PNG first, JPEG then smaller — instead of letting it be dropped after the draft was cleared; `attachNote` says why a file did not attach (undecodable/HEIC, the cap, a duplicate); the ANNOTATION EDITOR (`openAnnotator` / `drawShapes` / `closeAnnotator`, state in module-level `annot`, mounted on `document.body` outside `#root` so a repaint mid-stroke cannot take the stroke): box and arrow (numbered), pen, label, four colours, undo; flattened from the ORIGINAL into the bytes sent, vectors kept for re-edit; `annotationHint` tells the model the marks are the user's.
